...
def calculate_similarity_matrices(abstracts, type):
    cossim_matrix = np.zeros((len(abstracts), len(abstracts)), dtype=np.float64)
    jaccard_matrix = np.zeros((len(abstracts), len(abstracts)), dtype=np.float64)
    for i in range(len(abstracts)):
        for j in range(i+1,len(abstracts)):
            cossim_matrix[i,j] = cossim_matrix[j,i] = 1 - cosine_similarity(abstracts[i], abstracts[j], type)
            jaccard_matrix[i,j] = jaccard_matrix[j,i] = 1 - jaccard_index(abstracts[i], abstracts[j], type)
...
def master(comm, abstracts, type):
    ...
    size = comm.Get_size()
    status = MPI.Status()
    cossim_matrix = np.float64(np.zeros((len(abstracts), len(abstracts))))
    jaccard_matrix = np.float64(np.zeros((len(abstracts), len(abstracts))))
    ...
    for i in range(len(abstracts)):
        for j in range(i+1, len(abstracts)):
            if ind < size-1:
                ...
                cossim_matrix[x,y] = cossim_matrix[y,x] = 1 - cossim
                jaccard_matrix[x,y] = jaccard_matrix[y,x] = 1 - jaccard
